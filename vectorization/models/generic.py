...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bn=True, conv_class=nn.Conv1d):
        ...
        modules = (
            conv_class(in_channels, out_channels, kernel_size),
    ...
    def from_spec(cls, spec):
        ...
        if use_fc_from_conv:
            conv_adapter = ConvAdapter(
                spec["conv"]["convmap_channels"], spec["hidden"]["feature_dim"], bn=spec["conv"]["bn"]
            )
        else:
            conv_adapter = None
        ...
        if use_fc_from_hidden:
            transformer_adapter = TransformerAdapter(
                spec["hidden"]["feature_dim"], spec["output"]["in_features"], bn=False
            )
        else:
...
__all__ = ["GenericVectorizationNet"]
