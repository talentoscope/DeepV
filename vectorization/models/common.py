...
def vgg_model_creator(in_channels=1, convmap_channels=128, blocks=1, conf="A", bn=False):
    ...
    return _make_layers(cfg, batch_norm=bn, in_channels=in_channels, convmap_channels=convmap_channels, blocks=blocks)
...
class ConvFeatureExtractor(SpecifiedModuleBase):
    def __init__(self, in_channels=1, convmap_channels=128, kind="resnet", blocks=1, conf=None, **kwargs):
        ...
        self.conv = model_creator[kind](
            input_channels=in_channels, convmap_channels=convmap_channels, blocks=blocks, conf=conf, **kwargs
        )
