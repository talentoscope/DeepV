...
def main(options):
    ...
    IMGLOG_PARAMS = {
        ...
        "patch_size": (150, 150),
        "with_skeleton": True,
        "stack_grids_horizontally": False,
        "skeleton_node_size": 5,
    }
    ...
    train_loader_memory_constraint = (
        options.memory_constraint
    )  # TODO could be calculated more precisely with respect to the other memory requirements
    loader_params = dict(
    ...
    if options.dataloader_type == "handcrafted":
        loader_params.update(
            handcrafted_train_paths=options.handcrafted_train_paths,
    ...
    def validate(loader, log_i, prefix=""):
        ...
        for name, value in metrics_scalars.items():
            ...
            values_array[np.isnan(values_array)] = 0.0  # maybe tensorboard does handle this by itself, dunno
            values_array[np.isinf(values_array)] = 0.0  # otherwise error "The histogram is empty"
    ...
    for epoch_i in range(epochs_completed, epochs_completed + options.epochs):
        for batch_i, batch_data in islice(enumerate(train_loader), batches_completed_in_epoch, len(train_loader)):
            ...
            l2_weight = min(max(0.0, l2_weight + l2_weight_change), 1.0)
            ...
            if batch_i > 0 and batch_i % options.batches_before_val == 0:
                ...
                logger.info_scalars(
                    "Computed {key} over last batch of {num_items} images: {value:.4f}",
                    train_metrics,
...
def parse_args():
    ...
    parser.add_argument(
        "-B", "--val-batch-size", type=int, default=128, dest="val_batch_size", help="val batch size [default: 128]."
    )
    parser.add_argument(
        "--batches-before-val",
        type=int,
        default=1024,
        dest="batches_before_val",
        help="how many batches to train before validation [default: 1024].",
    )
    parser.add_argument(
        "--mini-val-batches-n-per-subset",
        type=int,
        default=12,
        dest="mini_val_batches_n_per_subset",
        help="how many batches per subset to run for mini validation [default: 12].",
    )

    parser.add_argument(
        "--model-spec",
        dest="model_spec_filename",
        required=True,
        help="model specification JSON file to use [default: none].",
    )
    parser.add_argument(
        "--infer-from-spec",
        dest="infer_from_spec",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--data-type",
        required=True,
        dest="dataloader_type",
        help="type of the train/val data to use.",
        choices=dataloading.prepare_loaders.keys(),
    )
    parser.add_argument(
        "--handcrafted-train",
        required=False,
        action="append",
        dest="handcrafted_train_paths",
        help="dirnames of handcrafted datasets used for training "
    ...
    parser.add_argument(
        ...
        required=False,
        action="append",
        dest="handcrafted_val_paths",
        help="dirnames of handcrafted datasets used for validation "
        "(sought for in preprocessed/synthetic_handcrafted).",
    )
    parser.add_argument(
        "--handcrafted-val-part",
        required=False,
        type=float,
        default=0.1,
        dest="handcrafted_val_part",
        help="portion of handcrafted_train used for validation",
    )
    parser.add_argument(
        "-M",
        "--memory-constraint",
        required=True,
        type=int,
        dest="memory_constraint",
        help="maximum RAM usage in bytes.",
    )

    parser.add_argument(
        "-r", "--render-resolution", dest="render_res", default=64, type=int, help="resolution used for rendering."
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", default=False, dest="verbose", help="verbose output [default: False]."
    )
    parser.add_argument(
        "-l",
        "--logging-file",
    ...
    parser.add_argument(
        "-tl",
        "--tboard-json-logging-file",
        dest="tboard_json_logging_file",
