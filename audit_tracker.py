...
class AuditTracker:
    ...
    def __init__(self, audit_file: str = "audit_progress.json"):
        ...
        self.exclude_dirs = {
            ...
            'data', 'logs', 'models', 'vectorization/models/specs',
            'notebooks', 'docs/_build', '.pytest_cache', '.mypy_cache',
            'dist', 'build', '*.egg-info'
        }
    ...
    def get_python_files(self) -> List[str]:
        ...
        for root, dirs, files in os.walk('.'):
            ...
            dirs[:] = [d for d in dirs if d not in self.exclude_dirs and
                       not any(pattern in d for pattern in ['__pycache__', '.git'])]

            for file in files:
                if file.endswith('.py'):
                    # Skip test files and other excluded patterns
                    if not any(pattern.replace('*', '').replace('_', '') in file.lower()
    ...
    def mark_file_audited(self, filepath: str, status: str = 'completed',
