...
def precompute_targets(split_file, raster_dir, vector_dir, cache_dir, max_primitives=20):
    ...
    cache_info = {"success": 0, "errors": 0, "error_files": []}
...
def main():
    ...
    parser.add_argument(
        "--cache-dir", type=str, default="data/cache/floorplancad", help="Directory to store cached primitives"
    )
    parser.add_argument("--max-primitives", type=int, default=20, help="Maximum number of primitives per image")
    parser.add_argument(
        "--splits", nargs="+", default=["train", "val", "test"], help="Splits to pre-compute (default: train val test)"
    )
    ...
    for split in args.splits:
        ...
        precompute_targets(split_file, args.raster_dir, args.vector_dir, split_cache_dir, args.max_primitives)
