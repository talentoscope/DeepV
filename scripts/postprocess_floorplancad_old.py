...
def process_png(svg_path: Path, output_path: Path):
    ...
    try:
        ...
        cairosvg.svg2png(
            bytestring=svg_content.encode("utf-8"), write_to=str(output_path), output_width=1000, output_height=1000
        )
...
def main():
    ...
    parser.add_argument(
        "--output_vector_dir", type=Path, help="Output directory for processed SVGs (default: overwrite)"
    )
    parser.add_argument(
        "--output_raster_dir", type=Path, help="Output directory for processed PNGs (default: overwrite)"
    )
