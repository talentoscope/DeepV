...
class ComprehensiveQualityAnalyzer:
    ...
    def vector_to_vector_comparison(self, ground_truth: np.ndarray, predicted: np.ndarray) -> Dict[str, Any]:
        ...
        if len(ground_truth) > 0 and len(predicted) > 0:
            ...
            metrics["stroke_width_mae"] = (
                np.mean(np.abs(gt_widths - pred_widths[: len(gt_widths)]))
                if len(pred_widths) >= len(gt_widths)
                else float("inf")
            )

        # Length comparison
        gt_lengths = np.sqrt((gt_coords[:, 2] - gt_coords[:, 0]) ** 2 + (gt_coords[:, 3] - gt_coords[:, 1]) ** 2)
        pred_lengths = np.sqrt(
            (pred_coords[:, 2] - pred_coords[:, 0]) ** 2 + (pred_coords[:, 3] - pred_coords[:, 1]) ** 2
        )

        if len(gt_lengths) > 0 and len(pred_lengths) > 0:
            metrics["mean_length_diff"] = abs(gt_lengths.mean() - pred_lengths.mean())
            metrics["length_mae"] = (
                np.mean(np.abs(gt_lengths - pred_lengths[: len(gt_lengths)]))
                if len(pred_lengths) >= len(gt_lengths)
                else float("inf")
            )
    ...
    def structural_topological_metrics(self, vectors: np.ndarray) -> Dict[str, Any]:
        ...
        lengths = np.sqrt(
            (vectors_sample[:, 3] - vectors_sample[:, 1]) ** 2 + (vectors_sample[:, 4] - vectors_sample[:, 2]) ** 2
        )
    ...
    def cad_specific_metrics(self, vectors: np.ndarray) -> Dict[str, Any]:
        ...
        if len(vectors_sample) > 1 and HAS_SCIPY:
            angles = np.arctan2(
                vectors_sample[:, 4] - vectors_sample[:, 2], vectors_sample[:, 3] - vectors_sample[:, 1]
            )
            angle_diffs = cdist(
                angles.reshape(-1, 1),
                angles.reshape(-1, 1),
