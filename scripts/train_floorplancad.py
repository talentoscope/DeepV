        ...
        for batch_idx, (images, targets, primitive_counts) in enumerate(train_pbar):
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                ...
                if hasattr(model.hidden, "count_predictor"):
                    ...
                    outputs = model(images)
                    mask = (
                        torch.arange(max_primitives, device=primitive_counts.device)[None, :]
...
def main():
    ...
    parser.add_argument(
        "--split-dir",
        type=str,
        default="data/splits/floorplancad",
        help="Directory containing train.txt, val.txt, test.txt",
    )
    parser.add_argument(
        "--raster-dir", type=str, default="data/raster/floorplancad", help="Directory containing raster PNG files"
    )
    parser.add_argument(
        "--vector-dir", type=str, default="data/vector/floorplancad", help="Directory containing vector SVG files"
    )
    parser.add_argument("--model-spec", type=str, required=True, help="Path to model specification JSON file")
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Path to model checkpoint file (optional, for resuming training)"
    )
    parser.add_argument("--batch-size", type=int, default=8, help="Batch size")
    parser.add_argument("--epochs", type=int, default=10, help="Number of epochs")
    parser.add_argument("--gpu", type=int, default=0, help="GPU device ID")
    parser.add_argument(
        "--model-output-count", type=int, default=10, help="Number of primitives to output (for fixed-length models)"
    )
    parser.add_argument(
        "--max-primitives", type=int, default=20, help="Maximum number of primitives for variable-length models"
    )
    ...
    parser.add_argument(
        "--resize", type=int, default=None, help="Resize images to square size (e.g., 512) to reduce memory usage"
    )
    parser.add_argument("--amp", action="store_true", help="Enable mixed precision training (reduces memory usage)")
    parser.add_argument(
        "--grad-accum-steps", type=int, default=1, help="Accumulate gradients across N steps to reduce memory usage"
    )
