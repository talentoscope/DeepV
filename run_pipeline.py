...
class PipelineRunner:
    ...
    def _process_single_image(self, image, image_index):
        ...
        if self.options.primitive_type == "curve":
            ...
            raise ValueError(
                f"Unsupported primitive type: {self.options.primitive_type}. " "Supported types are 'line' and 'curve'."
            )
    ...
    def _process_line_pipeline(self, patches_rgb, patches_vector, patches_offsets, input_rgb, image, tracer):
        ...
        print(f"[Pipeline] patches_rgb.shape={patches_rgb.shape}, " f"patches_vector.shape={patches_vector.shape}")
