...
def main(
    ...
    whole_image_size = intermediate_output[
        "cleaned_image_shape"
    ]  # FIXME: cleaned_image_shape is different from actual svg size
    ...
    if not options.init_random:
        ...
        batches, patch_offsets = group_patches(
            model_output, raster_patches, patch_offsets, min_confidence, primitives_n, batch_size
        )
        ...
        for first_patch_i in range(0, patches_n, batch_size):
            ...
            batches.append(
                ((next_first_patch_i - first_patch_i, primitives_n), raster_patches[first_patch_i:next_first_patch_i])
            )
    del raster_patches

    # 4. Define the function to assemble patches into VectorImage
    def get_vectorimage(patches):
        return build_vectorimage(
            patches,
            patch_offsets,
            whole_image_size,
            control_points_n,
    ...
    init_primitives = make_init_primitives(
        options.init_random,
        primitive_type,
        batches,
        patch_width,
            ...
            for i in trange(
                ...
                if i % measure_period == 0:
                    ...
                    rasterization = binarize(
                        aligner.prim_ten.render_with_cairo_total(patch_width, patch_height, min_width=min_width)
                    )
