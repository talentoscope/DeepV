...
MODEL_LOSS = {
    ...
    "SmallUnet": {"gen": SmallUnet(), "unet": SmallUnet(), "loss": CleaningLoss(kind="MSE", with_restore=False)},
}
...
def validate(tb, val_loader, unet, gen, loss_func, global_step):
    ...
    for x_input, y_extract, y_restor in tqdm(val_loader):
        ...
        with torch.no_grad():
            ...
            iou_scr_without_gan = iou_score(
                1 - torch.round(logits_extract.squeeze(1)).cpu().long().numpy(),
            ...
            iou_scr = iou_score(
                1 - torch.round(torch.clamp(logits_extract + logits_restore, 0, 1).squeeze(1).cpu()).long().numpy(),
                1 - torch.round(y_restor.squeeze(1)).cpu().long().numpy(),
            )
    ...
    input_grid = torchvision.utils.make_grid(1.0 - logits_extract.cpu())
    true_grid = torchvision.utils.make_grid(1.0 - y_restor.cpu())
