...
class ChunkedDatasetLoader:
    ...
    def __iter__(self):
        ...
        while self.data.next_size() > 0:
            ...
            if self.shuffle:
                ids = torch.randperm(len(inputs))
            else:
                ids = torch.arange(0, len(inputs))
            for b in (
                (inputs[ids[start : start + self.batch_size]], targets[ids[start : start + self.batch_size]])
                for start in torch.arange(0, len(ids), self.batch_size)
            ):
                yield b
        raise StopIteration
    ...
    def allocate_buffers(self):
        ...
        self.input_buffers = [
            torch.empty(max_chunk_size, *input_shape, dtype=input_type).share_memory_() for _ in (1, 2)
        ]
        self.target_buffers = [
            torch.empty(max_chunk_size, *target_shape, dtype=target_type).share_memory_() for _ in (1, 2)
        ]
