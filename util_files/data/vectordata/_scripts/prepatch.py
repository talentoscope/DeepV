...
def prepatch_sesyd(data_root, patches_root, only_walls=False, num_workers=-1):
    ...
    rotations += [
        base_rot + dev_rot * dev_sign
        for dev_rot in range(1, max_rot_deviation + 1, 1)
        for base_rot in (0, 90, 180, 270)
        for dev_sign in (1, -1)
    ]
...
def prepatch_pfp(data_root, patches_root, num_workers=-1):
    ...
    translations = (
        np.asarray(
            np.meshgrid(np.linspace(0, patch_w, 4 * 3, endpoint=False), np.linspace(0, patch_h, 4 * 3, endpoint=False))
        )
        .transpose(1, 2, 0)
        .reshape(-1, 2)
    )
