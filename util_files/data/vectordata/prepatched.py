...
class PrepatchedSVG(LineDrawingsDataset):
    ...
    def _get_vector_item(self, idx):
        ...
        lines, beziers = VectorImage.from_svg(file).vahe_representation(
            max_lines_n=self.max_primitives[PrimitiveType.PT_LINE],
            max_beziers_n=self.max_primitives[PrimitiveType.PT_QBEZIER],
            random_sampling=self.sample_primitives_randomly,
        )
