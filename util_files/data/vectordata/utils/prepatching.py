...
def make_patches(
    ...
    rotations=(0,),
    translations=((0, 0),),
    distinguishability_threshold=0.5,
    num_workers=0,
    random_samples=None,
    leave_width_percentile=None,
):
    if num_workers != 0:
    ...
    for file in source_images:
        ...
        if random_samples is not None:
            ...
            orientations = np.random.choice(
                orientations_all, size=min(random_samples, len(orientations_all)), replace=False
            )
                ...
                for rotation in rotations:
                    ...
                    for translation in translations:
                        ...
                        def simplify_and_save(
                            ...
                            save_path = path(
                                basename,
                                (int(vector_patch.x.as_pixels()), int(vector_patch.y.as_pixels())),
                                width,
                                orientation,
                                rotation,
                                translation,
