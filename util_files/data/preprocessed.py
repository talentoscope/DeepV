...
patch_parameters = {"patch_width": 64, "patch_height": 64, "max_lines": 10, "max_curves": 0}

svg_parameters = {"widths": ("*",), "orientations": ("*",), "rotations": ("*",), "translations": (("*", "*"),)}


class SynParameters:
    from util_files.data.syndata.utils import renormalize as _normalize_probas

    strokes_probas = _normalize_probas({1: 0.45, 2: 0.45, 3: 0.1})

    directions_probas = {}
    for angle in np.linspace(0, np.pi, 13):
        if angle in [0, np.pi / 2]:  # - horizontal, | vertical
            directions_probas[angle] = 0.30
        elif angle in [np.pi / 4, np.pi * 3 / 4]:  # /, \
            directions_probas[angle] = 0.15
        elif angle < np.pi:
            directions_probas[angle] = 0.1 / 8
...
syn_parameters = {
    ...
    "min_directions": 1,
    "max_directions": 2,
    "min_primitives_gap": 2,
    "max_primitives_gap": 10,
    "min_stroke_width": 1,
    "max_stroke_width": 7,
    "min_stroke_length": max(patch_parameters["patch_width"], patch_parameters["patch_height"])
    * 0.90
    * SynParameters.padding_factor,
    "max_stroke_length": np.sqrt(patch_parameters["patch_width"] ** 2 + patch_parameters["patch_height"] ** 2)
    * SynParameters.padding_factor,
    "primitives_endpoint_noise_sigma": 0.5,
    "primitives_direction_noise_sigma": np.pi / 270.0,
...
class _PreprocessedDataset(Dataset):
    def __init__(self, data_dir):
        ...
        self.images = torch.FloatTensor(
            torch.FloatStorage.from_file(images_file, size=np.prod(images_shape), shared=True)
        ...
        self.targets = torch.FloatTensor(
            torch.FloatStorage.from_file(targets_file, size=np.prod(targets_shape), shared=True)
        ).reshape(*targets_shape)
    ...
    def from_svgdataset(
        ...
        out_root,
        shuffle_seed=None,
        reproducibility_manager=None,
        preprocessed_svg_dataset_class=PreprocessedSVG,
    ):
        ...
        svg_dataset = preprocessed_svg_dataset_class(
            patch_size=(patch_parameters["patch_width"], patch_parameters["patch_height"]),
        ...
        if shuffle_seed is not None:
            svg_dataset = torch.utils.data.Subset(
                svg_dataset, np.random.RandomState(seed=shuffle_seed).permutation(len(svg_dataset))
            )
    ...
    @classmethod
    def from_synthetic_handcrafted(
        cls, patch_parameters, syn_parameters, topologies_with_probas, out_root, reproducibility_manager
    ):
        # make primitive_types and max_primitives dicts based on the parameters
        primitive_types = []
        max_primitives = {}
        ...
        syn_dataset = PreprocessedSyntheticHandcrafted(
            patch_size=(patch_parameters["patch_width"], patch_parameters["patch_width"]),
            normalize_image=True,
            primitive_types=primitive_types,
            max_primitives=max_primitives,
            size=syn_parameters["samples_n"],
            topologies_with_probas=topologies_with_probas,
            **syn_parameters,
        )
