...
def snap_beam(snap_what, snap_to, endpoint_type=ET_A):
    ...
    for line_prim, line_svg in zip(snapped_first, first_svg):
        for attempt in range(4):
            if ET_A == endpoint_type:
                line_svg.start = line_svg.point(1 - 2**attempt)
            elif ET_B == endpoint_type:
                line_svg.end = line_svg.point(2**attempt)
            intersection = line_svg.intersect(second_svg[-1])
            if intersection:
                t1, t2 = intersection[0]
                new_endpoint = c2t(line_svg.point(t1))
                if ET_A == endpoint_type:
                    line_prim.point1 = new_endpoint
                elif ET_B == endpoint_type:
                    line_prim.point2 = new_endpoint
                break
        if not intersection:
...
def snap_outer(snap_first, snap_second, endpoint_first=ET_A, endpoint_second=ET_A):
    ...
    for line_prim_first, line_svg_first, line_prim_second, line_svg_second in zip(
        snapped_first, first_svg, snapped_second, second_svg
    ):
        for attempt in range(4):
            if ET_A == endpoint_first:
                line_svg_first.start = line_svg_first.point(1 - 2**attempt)
            elif ET_B == endpoint_first:
                line_svg_first.end = line_svg_first.point(2**attempt)

            if ET_A == endpoint_second:
                line_svg_second.start = line_svg_second.point(1 - 2**attempt)
            elif ET_B == endpoint_second:
                line_svg_second.end = line_svg_second.point(2**attempt)
            ...
            if intersection:
                ...
                if ET_A == endpoint_first:
                    line_prim_first.point1 = new_endpoint
                elif ET_B == endpoint_first:
                    line_prim_first.point2 = new_endpoint

                if ET_A == endpoint_second:
                    line_prim_second.point1 = new_endpoint
                elif ET_B == endpoint_second:
                    line_prim_second.point2 = new_endpoint
                break
