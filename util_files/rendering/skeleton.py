...
def make_skeleton_vahe(primitive_sets, line_width, node_size, control_line_width, control_node_size):
    edges = {
        prim_type: [_get_edges_vahe[prim_type](prim, line_width) for prim in prims]
    ...
    nodes = {
        PT_POINT: [
            ...
            for prim_type, prims in primitive_sets.items()
            for prim in prims
            for node in _get_nodes_vahe[prim_type](prim, node_size)
        ]
    }
...
def get_qbezier_b_controls_vahe(bezier, line_width, node_size):
    ...
    p2 = bezier[4:6]
    return {
        PT_POINT: bezier_controls[PT_POINT] + [b],
        PT_LINE: (
            bezier_controls[PT_LINE] + [np.array([*p0, *b[:2], line_width]), np.array([*b[:2], *p2, line_width])]
        ),
    }


_get_edges_vahe = {
    PT_LINE: get_line_edge_vahe,
    PT_CBEZIER: get_bezier_edge_vahe,
    PT_QBEZIER: get_qbezier_edge_vahe,
    PT_QBEZIER_B: get_qbezier_edge_vahe,
}
_get_nodes_vahe = {
    PT_LINE: get_line_nodes_vahe,
    PT_CBEZIER: get_bezier_nodes_vahe,
    PT_QBEZIER: get_qbezier_nodes_vahe,
    PT_QBEZIER_B: get_qbezier_b_nodes_vahe,
}
_get_controls_vahe = {
    PT_LINE: get_line_controls_vahe,
    PT_CBEZIER: get_bezier_controls_vahe,
    PT_QBEZIER: get_qbezier_controls_vahe,
    PT_QBEZIER_B: get_qbezier_b_controls_vahe,
}
