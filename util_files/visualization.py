...
#     'vectran.train.diff_rendering is deprecated, '
#     'use vectran.renderers.differentiable_rendering.'
#     'sigmoids_renderer.renderer.Renderer'
# )
...
def _get_items_by_ids(loader, ids: Iterable):
    ...
    return (
        inputs[ids_back_to_original_order].contiguous(),
        targets[ids_back_to_original_order].contiguous(),
    )


def get_ranked_subset_ids(
    scores: Iterable[Number], subset_size: int
    ...
    average_ids = np.argpartition(np.abs(scores - scores.mean()), subset_size)[
        :subset_size
    ]
...
def make_ranked_images_from_loader_and_model(
    ...
    **kwargs,
    ...
    #         # optimizer = torch.optim.Adam([y_pred_nowc], betas=(0.9, 0.98),
    #         #                                eps=1e-09, lr=0.001)
    ...
    #             # y_pred_render = torch.cat((y_pred_nowc * 64,
    #             #                             y_pred[..., -2:-1] * 64,
    #             #                             y_pred[..., 5:]), dim=-1)
    #             y_pred_render = torch.cat((y_pred_nowc[..., :5] * 64,
    #                                        y_pred_nowc[..., 5:]), dim=-1)

    #             images_pred = diff_rendering.render(y_pred_render, (64, 64),
    #                                                sigmoid_rate=10)
    ...
    worst_grid = make_images(
        outputs[worst_ids].reshape(lines_shape),
        targets[worst_ids].reshape(lines_shape),
        **kwargs,
    )
    best_grid = make_images(
        outputs[best_ids].reshape(lines_shape),
        targets[best_ids].reshape(lines_shape),
        **kwargs,
    )
    average_grid = make_images(
        outputs[average_ids].reshape(lines_shape),
        targets[average_ids].reshape(lines_shape),
        **kwargs,
    )
...
def make_ranked_images(lines_out, lines_gt, imgrid_shape, ranks, **kwargs):
    ...
    average_ids = np.argpartition(np.abs(np.asarray(ranks) - np.mean(ranks)), images_n)[
        :images_n
    ]
    ...
    worst_grid = make_images(
        lines_out[worst_ids].reshape(lines_shape),
        lines_gt[worst_ids].reshape(lines_shape),
        **kwargs,
    )
    best_grid = make_images(
        lines_out[best_ids].reshape(lines_shape),
        lines_gt[best_ids].reshape(lines_shape),
        **kwargs,
    )
    average_grid = make_images(
        lines_out[average_ids].reshape(lines_shape),
        lines_gt[average_ids].reshape(lines_shape),
        **kwargs,
    )
...
def make_images(
    ...
    if stack_grids_horizontally:
        grid_origins = (
            np.array([0, imgrid_width + grid_padding]) * np.arange(3)[..., None]
        )
        imgrid_width = imgrid_width * 3 + grid_padding * 2
    else:
        grid_origins = (
            np.array([imgrid_height + grid_padding, 0]) * np.arange(3)[..., None]
        )
    ...
    for grid_origin_i, grid_origin_j in grid_origins:
        ...
        for patch_i in range(1, patches_n_vert):
            ...
            subgrid[
                patch_origin_i - padding_vert : patch_origin_i
            ] = patch_padding_color
        for patch_j in range(1, patches_n_hor):
            patch_origin_j = (patch_width + padding_hor) * patch_j
            subgrid[
                :, patch_origin_j - padding_hor : patch_origin_j
            ] = patch_padding_color
        ...
        for grid_origin_i, grid_origin_j in grid_origins[1:]:
            if stack_grids_horizontally:
                imgrid[
                    :, grid_origin_j - grid_padding : grid_origin_j
                ] = grid_padding_color
            else:
                imgrid[
                    grid_origin_i - grid_padding : grid_origin_i
                ] = grid_padding_color
    ...
    if with_skeleton:
        ...
        def renderer(lines):
            return render_without_skeleton(
                lines, patch_size=patch_size, primitive_type=pt
            )[..., None]

    # render outs
    grid_origin_i, grid_origin_j = grid_origins[0]
    render_lines_to(
        imgrid[grid_origin_i:, grid_origin_j:],
        lines_out,
        renderer,
        patch_size,
        patch_padding,
    ...
    # render overlay
    if with_skeleton:
        def renderer(lines_out, lines_gt):
            return render_overlay_colored(
...
def render_without_skeleton(
    lines, patch_size, primitive_type=graphics_primitives.PrimitiveType.PT_LINE
):
    scaled_primitives = postprocess_primitives(lines, patch_size[0])
    # TODO fix
    return _render(
        {primitive_type: scaled_primitives}, patch_size, data_representation="vahe"
    )
...
def render_overlay(
    ...
    # TODO fix
    out_image = _render(
        {primitive_type: scaled_out}, patch_size, data_representation="vahe"
    )
    gt_image = _render(
        {primitive_type: scaled_gt}, patch_size, data_representation="vahe"
    )

    return np.uint8(255) - (
        (np.uint8(255) - out_image) // np.uint8(2)
        + (np.uint8(255) - gt_image) // np.uint8(2)
    )


def render_overlay_colored(
    lines_out,
    lines_gt,
    patch_size,
    primitive_type=graphics_primitives.PrimitiveType.PT_LINE,
...
def render_lines_to(imgrid, lines, renderer, patch_size, patch_padding):
    ...
    for patch_i in range(patches_n_vert):
        ...
        for patch_j in range(patches_n_hor):
            ...
            imgrid[
                i_start : i_start + rendering_h, j_start : j_start + rendering_w
            ] = rendering


def render_line_pairs_to(
    imgrid, lines_out, lines_gt, renderer, patch_size, patch_padding
):
    patch_width, patch_height = patch_size
    patches_n_vert, patches_n_hor = lines_out.shape[:2]
    padding_hor, padding_vert = patch_padding
    ...
    for patch_i in range(patches_n_vert):
        ...
        for patch_j in range(patches_n_hor):
            ...
            rendering = renderer(
                lines_out[patch_i, patch_j], lines_gt[patch_i, patch_j]
            )
            rendering_h, rendering_w = rendering.shape[:2]
            imgrid[
                i_start : i_start + rendering_h, j_start : j_start + rendering_w
