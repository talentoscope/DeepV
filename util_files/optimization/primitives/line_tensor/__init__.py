...
from util_files.rendering.cairo import PT_LINE, render_with_skeleton
...
class LineTensor(PrimitiveTensor):
    def __init__(self, p1, p2, width, dtype=None, device=None):
        ...
        self.canonical_parameters = [
            {"parameter": self._midpoint, "lr_factor": 1},
    ...
    def constrain_parameters(self, patch_width, patch_height, division_epsilon=division_epsilon):
        ...
        if isinstance(optimizer, torch.optim.Adam) and "exp_avg" in optimizer.state[l]:
            ...
            optimizer.state[l]["exp_avg_sq"].data[dwarf_lines], optimizer.state[w]["exp_avg_sq"].data[dwarf_lines] = (
                optimizer.state[w]["exp_avg_sq"].data[dwarf_lines],
    ...
    def get_vector_field_at(self, t):
        ...
        vector_field = vector_field.reshape(patches_n, spatial_dims_n, primitives_n, 1).expand(
            patches_n, spatial_dims_n, primitives_n, pixels_n
        )
        return vector_field
    ...
    def render_skeleton_total(
        ...
        widths = widths * 0
        pars = np.concatenate([self.p1.detach().cpu().numpy(), self.p2.detach().cpu().numpy(), widths], 1).swapaxes(
            1, 2
        )
        pars *= scaling
        return np.stack(
    ...
    def unit_energy(self, pixel_coords):
        ...
        if (
            (self._unit_energy is not None)
            and (self._pixel_coords is not None)
            and torch.all(self._pixel_coords == pixel_coords)
        ):
