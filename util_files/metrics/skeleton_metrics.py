...
def sample_points(vector_image: VectorImage, points_n=None, point_density=None, dtype=np.float128):
    ...
    points = np.concatenate(
        [
            sample_points_at_arclengths(
                segments[seg_i],
                pointcloud_arc_coordinates[fist_point_on_segment[seg_i] : fist_point_on_segment[seg_i + 1]]
                - cumsum[seg_i],
                dtype=dtype,
            )
...
def sample_points_at_arclengths(segment, arclengths, dtype=np.float128, ilength_tol=1e-2):
    if isinstance(segment, svgpathtools.Line):
        ...
        ts = np.asarray(
            list(map(lambda s: segment.ilength(np.clip(s, 0, segment.length()), s_tol=ilength_tol), arclengths)),
            dtype=dtype,
        )
...
def cpch_distance(
    vector_image1: VectorImage,
    vector_image2: VectorImage,
    points_per_unit_length_in_pixels=3,
    ...
    return {
        "Chamfer distance in pixels squared": (dist1_squared.sum() + dist2_squared.sum()).item(),
