...
def render(data, dimensions, data_representation="paths", linecaps="butt", linejoin="miter"):
    return original_render(
        data, dimensions, data_representation=data_representation, linecaps=linecaps, linejoin=linejoin
    )


def iou_vector_reference(
    output_vector_image: VectorImage, reference_vector_image: VectorImage, width="mean", binarization="median"
):
    ...
    if (width == "mean") or isinstance(width, (int, float)):
        if width == "mean":
            width_to_set = np.array(
                [float(path.width.as_pixels()) for path in reference_vector_image.paths if path.width is not None]
            ).mean()
        else:
            width_to_set = width
...
def iou_raster_reference(
    output_vector_image: VectorImage, reference_raster_image_np: np.ndarray, width="mean", binarization="median"
):
    output_vector_image = output_vector_image.with_filled_removed()
